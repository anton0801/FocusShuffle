...
final class WebCoordinator: NSObject {
    ...
    private let cookieJar = "focusshuffle_cookies"
    
    func loadURL(_ url: URL, in webView: WKWebView) {
        print("🎯 [FocusShuffle] Load: \(url.absoluteString)")
        redirectCount = 0
...
extension WebCoordinator: WKNavigationDelegate {
    ...
    func webView(_ webView: WKWebView, didCommit navigation: WKNavigation!) {
        if let current = webView.url { checkpoint = current; print("✅ [FocusShuffle] Commit: \(current.absoluteString)") }
    }
